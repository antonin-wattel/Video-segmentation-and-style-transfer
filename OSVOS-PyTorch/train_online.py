...
def train(seq_name='blackswan', save_dir_res=None, nAveGrad=5, nEpochs=50*5):
    ...
    #nEpochs = 50 * nAveGrad  # Number of epochs for training
    ...
    for epoch in range(0, nEpochs):
        ...
        for ii, sample_batched in enumerate(trainloader):
            ...
            # Print stuff
            #if epoch % (nEpochs//60) == (nEpochs//20 - 1):
            #print(epoch)
            #print(nEpochs//200)
    ...
    with torch.no_grad():  # PyTorch 0.4.0 style
        ...
        for ii, sample_batched in enumerate(testloader):
            ...
            for jj in range(int(inputs.size()[0])):
                ...
                #sm.imsave(os.path.join(save_dir_res, os.path.basename(fname[jj]) + '.png'), pred) #deprecated
                imageio.imwrite(os.path.join(save_dir_res, os.path.basename(fname[jj]) + '.png'), pred)

                #THIS WILL BE USELESS FOR US...
                if vis_res:
